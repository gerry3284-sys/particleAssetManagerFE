...
export class ReturnReceiptPdfService {
  async generate(data: ReturnReceiptPdfData): Promise<void> {
    const { doc } = await this.buildDocument(data);
    doc.save(this.buildFileName(data));
  }

  async generateBase64(data: ReturnReceiptPdfData): Promise<string> {
    const { doc } = await this.buildDocument(data);
  ...
  private async buildDocument(data: ReturnReceiptPdfData): Promise<{ doc: import('jspdf').jsPDF }> {
    ...
    doc.text('Ricevuta di Riconsegna Asset', marginX, y);
    ...
    const assignmentDateText = this.formatDate(data.assignmentDate);
    const returnDateText = this.formatDate(data.returnDate);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(12);
    doc.setTextColor(75, 85, 99);
    doc.text(
      `Gentile ${data.userName}, hai riconsegnato un asset aziendale in data ${returnDateText}.`,
      marginX,
      y
    );
    y += 22;
    ...
    y = this.drawSection(doc, {
      ...
      rows: [
        ...
        { label: 'Data di Assegnazione', value: assignmentDateText },
        { label: 'Data di Riconsegna', value: returnDateText },
        { label: 'Note di Riconsegna', value: data.returnNotes?.trim() || 'Nessuna nota' }
      ]
    });
  ...
  private buildFileName(data: ReturnReceiptPdfData): string {
    const date = this.formatDate(data.returnDate).replaceAll('/', '-');
    const serial = (data.serialNumber || 'asset').replace(/\s+/g, '-');
    return `ricevuta-riconsegna-${serial}-${date}.pdf`;
  }
}
