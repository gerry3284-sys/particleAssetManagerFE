...
export interface AssignmentReceiptPdfData {
  assetType: string;
  brand: string;
  model: string;
  serialNumber: string;
...
export class AssignmentReceiptPdfService {
  async generate(data: AssignmentReceiptPdfData): Promise<void> {
    const { doc } = await this.buildDocument(data);
    doc.save(this.buildFileName(data));
  }

  async generateBase64(data: AssignmentReceiptPdfData): Promise<string> {
    const { doc } = await this.buildDocument(data);
    const dataUri = doc.output('datauristring');
    return dataUri.split(',')[1] || '';
  }

  private async buildDocument(data: AssignmentReceiptPdfData): Promise<{ doc: import('jspdf').jsPDF }> {
    ...
    doc.text('Ricevuta di Assegnazione Asset', marginX, y);
    ...
    const prettyDate = this.formatDate(data.assignmentDate);
    doc.text(
      `Gentile ${data.userName}, ti e stato assegnato un asset aziendale in data ${prettyDate}.`,
      marginX,
      y
    );
    y += 22;
  ...
  private buildFileName(data: AssignmentReceiptPdfData): string {
    const date = this.formatDate(data.assignmentDate).replaceAll('/', '-');
    const serial = (data.serialNumber || 'asset').replace(/\s+/g, '-');
    return `ricevuta-assegnazione-${serial}-${date}.pdf`;
  }
}
